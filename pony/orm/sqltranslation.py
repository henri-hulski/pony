...
import ast, types, sys, re, inspect
...
from pony.orm.dbproviders.sqlite import SQLiteTranslator
from typing import Any, ClassVar, Generic, NoReturn, TypeVar, cast, overload
from typing_extensions import LiteralString, Self, TypeAlias

_Ast: TypeAlias = list["str | list[str | int | bool | list]"]
_Conditions: TypeAlias = list["list[list[Sequence[str]]]"]
_ExprMonadType: TypeAlias = (
    "type[Numeric | str | datetime | date | time | timedelta | Json | Array | Entity]"
)
_Sql: TypeAlias = list[
    "str | bool | type[str | int | float | bool | None] | list | None"
]
...
class SQLTranslator(ASTTranslator):
    ...
    database: Database | _typeshed.MaybeNone
    distinct: bool
    expr_columns: list[list[str]]
    expr_monads: Sequence[Monad]
    expr_type: ExprType
    extractors: dict[str, Callable] | _typeshed.MaybeNone
    filter_num: int
    ...
    row_layout: list[tuple[Callable, int | slice, str]] | _typeshed.MaybeNone
    sqlquery: SqlQuery
    tableref: TableRef | JoinedTableRef | ExprJoinedTableRef
    vars: QueryVars | None
    vartypes: VarTypes | _typeshed.MaybeNone

    dialect: ClassVar[str | _typeshed.MaybeNone] = None
    json_path_wildcard_syntax: ClassVar[bool] = False
    json_values_are_comparable: ClassVar[bool] = True
    row_value_syntax: ClassVar[bool] = True
    rowid_support: ClassVar[bool] = False
    ...
    def dispatch_external(translator, node: ast.AST) -> None:
        ...
        if t is NoneType:
            ...
            assert isinstance(t, QueryType)
            prev_translator = t.translator.deepcopy()
            setattr(prev_translator, "parent", translator)
            ...
            for i, item_type in enumerate(t):
                ...
                param = ParamMonad.new(
                    cast(PyType | EntityMeta, item_type), (varkey, i, None)
                )
                params.append(param)
            monad = ListMonad(params)
            if is_array:
                array_type = array_types.get(
                    cast(type[int | str | float], item_type), None
                )
            ...
            monad = ParamMonad.new(cast(PyType | EntityMeta, t), (varkey, None, None))
        node.monad = monad
        monad.node = node
        monad.aggregated = monad.nogroup = False

    def call(translator, method: Callable, node: ast.AST) -> Monad | None:
        try:
            monad = method(translator, node)
        except Exception:
            exc_class, exc, tb = sys.exc_info()
            try:
                if exc and not exc.args:
    ...
    def deepcopy(translator) -> SQLTranslator:
        ...
        setattr(result, "copied_from", translator)
        return result
    ...
    def init(
        ...
        if parent_translator is None:
            ...
            translator.sqlquery = SqlQuery(
                translator, parent_translator.sqlquery, left_join=left_join
            )
        ...
        for i, generator in enumerate(tree.generators):
            ...
            for ass_name in ass_names:
                ...
                assert isinstance(ass_name, ast.Name)
                if not isinstance(ass_name.ctx, ast.Store):
                    throw(TypeError, ast2src(ass_name))

            names = tuple(
                ass_name.id for ass_name in cast(tuple[ast.Name, ...], ass_names)
            )
            for name in names:
                if (
                    name in translator.namespace
                    and name in translator.sqlquery.tablerefs
                ):
            ...
            if monad:  # Lambda was encountered inside generator
                ...
                if isinstance(monad, EntityMonad):
                    ...
                    translator.sqlquery = monad._subselect(
                        translator.sqlquery, extract_outer_conditions=False
                    )
                ...
                if isinstance(iterable, SetType):
                    ...
                    translator.sqlquery.tablerefs[name] = tableref
                    tableref.make_join()
                    translator.namespace[name] = node.monad = ObjectIterMonad(
                        tableref, entity
                    )
                    ...
                    try:
                        ...
                        translator = cast(SQLTranslator, e.translator)
                ...
                if isinstance(monad, QuerySetMonad):
                    ...
                    for j, attrname in enumerate(attr_names):
                        ...
                        if j == last_index:
                            ...
                            name_path += "-" + attr.name
                        tableref = translator.sqlquery.add_tableref(
                            name_path, parent_tableref, attr
                        )
                        tableref.make_join(pk_only=True)
                        if j == last_index:
                            translator.namespace[name] = ObjectIterMonad(
        ...
        assert not isinstance(monad, ParamMonad)
        translator.expr_monads = (
            monad.items if isinstance(monad, ListMonad) else [monad]
        )
        ...
        if isinstance(expr_type, EntityMeta):
            assert isinstance(monad, (ObjectMixin, AttrSetMonad))
            entity = expr_type
            ...
            for m in expr_monads:
                ...
                if isinstance(expr_type, EntityMeta):
                    ...
                    def func(values, constructor=expr_type._get_by_raw_pkval_):  # type: ignore
                        if None in values:
                            return None
    ...
    @property
    def namespace(translator) -> dict[str, Monad | MonadMixin]:
        ...
        if (
            ...
            for name, base_expr_monad in zip(names, prev_translator.expr_monads):
                ...
                if isinstance(t, EntityMeta):
                    ...
                    assert base_expr_monad.nullable is not None
                    expr_monad = ExprMonad.new(
                        cast(_ExprMonadType, t),
                        cast(_Sql, expr_ast),
                        base_expr_monad.nullable,
                    )
    ...
    def construct_subquery_ast(
        ...
        assert isinstance(select_ast, list)
        ...
        if len(from_ast[1]) == 4:
            assert isinstance(from_ast, list) and isinstance(from_ast[1], list)
            outer_conditions = from_ast[1][-1]
            from_ast[1] = from_ast[1][:-1]
            assert isinstance(where_ast, list)
    ...
    def construct_sql_ast(
        ...
        if aggr_func_name:
            ...
            isinstance(translator.expr_type, EntityMeta)
            and not translator.parent
            and not translator.aggregated
            and not translator.optimize
        ):
        ...
        limit, offset = combine_limit_and_offset(
            translator.limit, translator.offset, limit, offset
        )
    ...
    def construct_delete_sql_ast(
        ...
        if (
            not force_in
            and len(from_ast) == 2
            and not translator.sqlquery.used_from_subquery
        ):
    ...
    def apply_lambda(
        ...
        translator.vars = vars
        translator.vartypes = (
            translator.vartypes.copy()
        )  # make HashableDict mutable again
        translator.vartypes.update(vartypes)

        if not original_names:
            assert argnames
            namespace = {
                name: monad for name, monad in zip(argnames, translator.expr_monads)
            }
        elif argnames:
    ...
    def postExpr(translator, node: ast.Expr) -> ExprMonad:
        return node.value.monad

    def preCompare(translator, node: ast.Compare) -> BoolMonad:
        monads = []
    ...
    def postConstant(translator, node: ast.Constant) -> ConstMonad | ListMonad:
        value = node.value
    ...
    def resolve_name(translator, name: str) -> Monad:
        if name not in translator.namespace:
            throw(
                TranslationError,
                "Name %s is not found in %s" % (name, translator.namespace),
            )
    ...
    def postAdd(translator, node: ast.BinOp) -> ExprMonad:
        return node.left.monad + node.right.monad

    def postSub(
        translator, node: ast.BinOp
    ) -> NumericExprMonad | TimedeltaExprMonad | DatetimeExprMonad | DateExprMonad:
        return node.left.monad - node.right.monad

    def postMult(translator, node: ast.BinOp) -> NumericExprMonad:
        return node.left.monad * node.right.monad

    def postMatMult(translator, node):
        throw(NotImplementedError)

    def postDiv(translator, node: ast.BinOp) -> NumericExprMonad:
        return node.left.monad / node.right.monad
    ...
    def postPow(translator, node: ast.BinOp) -> NumericExprMonad:
        return node.left.monad**node.right.monad

    def postUSub(translator, node: ast.UnaryOp) -> NumericExprMonad:
        return -node.operand.monad

    def postAttribute(translator, node: ast.Attribute) -> Monad:
        return node.value.monad.getattr(node.attr)

    def postAnd(translator, node: ast.BoolOp) -> AndMonad:
        return AndMonad([expr.monad for expr in node.values])

    def postOr(translator, node: ast.BoolOp) -> OrMonad:
    ...
    def postNot(translator, node: ast.UnaryOp) -> BoolMonad:
        return node.operand.monad.negate()

    def preCall(translator, node: ast.Call) -> Monad | None:
    ...
    def postCall(translator, node: ast.Call) -> Monad:
        args = []
        kwargs = {}
        for arg in node.args:
            if isinstance(arg, ast.Starred):
    ...
    def postSubscript(translator, node: ast.Subscript) -> Monad:
    ...
    def postIfExp(translator, node: ast.IfExp) -> ExprMonad:
        ...
        result_type = cast(
            _ExprMonadType, coerce_types(then_monad.type, else_monad.type)
        )
    ...
    def postFormattedValue(translator, node: ast.FormattedValue) -> AttrMonad:
...
class SqlQuery(object):
    ...
    def __init__(
        ...
        if parent_sqlquery is None:
            ...
            sqlquery.expr_counter = _count(1)
        else:
...
class TableRef(object):
    ...
    def __init__(tableref, sqlquery: SqlQuery, name: str, entity: EntityMeta) -> None:
    ...
    def make_join(tableref, pk_only: bool = False) -> tuple[str, list[str] | None]:
        ...
        if not tableref.joined:
            ...
            sqlquery.from_ast.append(
                [tableref.alias, "SELECT", tableref.subquery_ast[1:]]
            )
            tableref.joined = True
...
class StarTableRef(TableRef):
    ...
    def __init__(
        tableref, sqlquery: SqlQuery, name: str, entity: EntityMeta, subquery_ast: _Ast
    ) -> None:
...
class ExprJoinedTableRef(object):
    ...
    pk_columns: list[str] | _typeshed.MaybeNone
    sqlquery: SqlQuery
    used_attrs: set[Attribute]
    ...
    def make_join(tableref, pk_only: bool = False) -> tuple[str, list[str]]:
        ...
        join_cond = join_tables(
            parent_alias, tableref.alias, tableref.parent_columns, tableref.pk_columns
        )
...
class JoinedTableRef(object):
    ...
    def make_join(tableref, pk_only: bool = False) -> tuple[str, list[str]]:
        ...
        if not attr.is_collection:
            if not attr.columns:
                ...
                if attr.pk_offset is not None:
                    assert left_pk_columns is not None
        ...
        if (
            translator.optimize == tableref.name_path
            and translator.from_optimized
            and tableref.sqlquery is translator.sqlquery
        ):
            pass
        else:
...
class MonadMeta(type):
    ...
    tableref: TableRef | JoinedTableRef | ExprJoinedTableRef | _typeshed.MaybeNone
    type: Any
...
class Monad(object, metaclass=MonadMeta):
    node: ast.AST | _typeshed.MaybeNone
    nogroup: bool
    translator: SQLTranslator

    disable_distinct: ClassVar[bool] = False
    disable_ordering: ClassVar[bool] = False

    def __init__(monad, type: VarType, nullable: bool = True) -> None:
        monad.node = None
    ...
    def count(monad, distinct: NumericConstMonad | None = None) -> ExprMonad:
        distinct = distinct_from_monad(distinct, default=True)  # type: ignore
        ...
        result = ExprMonad.new(
            int, cast(_Sql, ["COUNT", distinct, expr]), nullable=False
        )
    ...
    def aggregate(
        ...
        distinct = distinct_from_monad(distinct)  # type: ignore
        translator = monad.translator
        if monad.aggregated:
            throw(
                TranslationError, "Aggregated functions cannot be nested. Got: {EXPR}"
        ...
        if func_name == "AVG":
            ...
            result_type = cast(_ExprMonadType, expr_type)
        if distinct is None:
            distinct = getattr(monad, "forced_distinct", False) and func_name in (
                "SUM",
                "AVG",
            )  # type: ignore
    ...
    def to_int(monad) -> NumericExprMonad | NoneMonad:
        return NumericExprMonad(
            int, ["TO_INT", monad.getsql()[0]], nullable=monad.nullable
        )

    def to_str(monad) -> StringExprMonad | NoneMonad:
        return StringExprMonad(
            str, ["TO_STR", monad.getsql()[0]], nullable=monad.nullable
        )

    def to_real(monad) -> NumericExprMonad | NoneMonad:
        return NumericExprMonad(
            float, ["TO_REAL", monad.getsql()[0]], nullable=monad.nullable
        )
...
def distinct_from_monad(distinct, default=None) -> bool | None:
    if distinct is None:
        return default
...
class MethodMonad(Monad):
    ...
    def getattr(monad, attrname) -> Any:
...
class EntityMonad(Monad):
    def __init__(monad, entity: EntityMeta) -> None:
...
class ListMonad(Monad):
    ...
    def __init__(monad, items: list) -> None:
    ...
    def contains(monad, x: Monad, not_in: bool = False) -> BoolExprMonad:
        ...
        if len(left_sql) == 1:
            if not_in:
                sql = [
                    "NOT_IN",
                    left_sql[0],
                    [item.getsql()[0] for item in monad.items],
                ]
            else:
        ...
        return BoolExprMonad(
            sql, nullable=x.nullable or any(item.nullable for item in monad.items)
        )
...
class NumericMixin(MonadMixin):
    ...
    __add__: Callable[[Monad, Monad], Monad] = make_numeric_binop("+", "ADD")
    __sub__: Callable[[Monad, Monad], Monad] = make_numeric_binop("-", "SUB")
    __mul__: Callable[[Monad, Monad], Monad] = make_numeric_binop("*", "MUL")
    __truediv__: Callable[[Monad, Monad], Monad] = make_numeric_binop("/", "DIV")
    __floordiv__: Callable[[Monad, Monad], Monad] = make_numeric_binop("//", "FLOORDIV")
    __mod__: Callable[[Monad, Monad], Monad] = make_numeric_binop("%", "MOD")
    __and__: Callable[[Monad, Monad], Monad] = make_numeric_binop("&", "BITAND")
    __or__: Callable[[Monad, Monad], Monad] = make_numeric_binop("|", "BITOR")
    __xor__: Callable[[Monad, Monad], Monad] = make_numeric_binop("^", "BITXOR")

    def __pow__(monad, monad2: Monad) -> Monad:
        if not isinstance(monad2, NumericMixin):
            throw(
                TypeError,
                _binop_errmsg % (type2str(monad.type), type2str(monad2.type), "**"),
            )
...
def make_datetime_binop(
    ...
    def datetime_binop(
        monad: DateMixin, monad2: TimedeltaMixin
...
class DateMixin(MonadMixin):
    ...
    attr_year: ClassVar[Callable[[Monad], NumericExprMonad]] = numeric_attr_factory(
        "YEAR"
    )
    attr_month: ClassVar[Callable[[Monad], NumericExprMonad]] = numeric_attr_factory(
        "MONTH"
    )
    attr_day: ClassVar[Callable[[Monad], NumericExprMonad]] = numeric_attr_factory(
        "DAY"
    )
    ...
    def __add__(monad, other: TimedeltaMixin) -> DateExprMonad | DatetimeExprMonad:
        if other.type != timedelta:
            throw(
                TypeError,
                _binop_errmsg % (type2str(monad.type), type2str(other.type), "+"),
            )
        return DateExprMonad(
            monad.type,
            ["DATE_ADD", monad.getsql()[0], other.getsql()[0]],
            nullable=monad.nullable or other.nullable,
        )

    def __sub__(
        monad, other: TimedeltaMixin | DateMixin
    ) -> TimedeltaExprMonad | DateExprMonad | DatetimeExprMonad:
        if other.type == timedelta:
            return DateExprMonad(
                monad.type,
                ["DATE_SUB", monad.getsql()[0], other.getsql()[0]],
                nullable=monad.nullable or other.nullable,
            )
        elif other.type == date:
            return TimedeltaExprMonad(
                timedelta,
                ["DATE_DIFF", monad.getsql()[0], other.getsql()[0]],
                nullable=monad.nullable or other.nullable,
            )
        throw(
            TypeError, _binop_errmsg % (type2str(monad.type), type2str(other.type), "-")
        )
...
class TimeMixin(MonadMixin):
    ...
    attr_hour: ClassVar[Callable[[Monad], NumericExprMonad]] = numeric_attr_factory(
        "HOUR"
    )
    attr_minute: ClassVar[Callable[[Monad], NumericExprMonad]] = numeric_attr_factory(
        "MINUTE"
    )
    attr_second: ClassVar[Callable[[Monad], NumericExprMonad]] = numeric_attr_factory(
        "SECOND"
    )
...
class DatetimeMixin(DateMixin):
    ...
    def call_date(monad) -> DateExprMonad:
        ...
        date_cast = ExprMonad.new(date, sql, nullable=monad.nullable)
        return cast(DateExprMonad, date_cast)
    ...
    def __add__(monad, other: TimedeltaMixin) -> DatetimeExprMonad:
        ...
        return DatetimeExprMonad(
            monad.type,
            ["DATETIME_ADD", monad.getsql()[0], other.getsql()[0]],
            nullable=monad.nullable or other.nullable,
        )
    ...
    def __sub__(
        ...
        if other.type == timedelta:
            return DatetimeExprMonad(
                monad.type,
                ["DATETIME_SUB", monad.getsql()[0], other.getsql()[0]],
                nullable=monad.nullable or other.nullable,
            )
        elif other.type == datetime:
            return TimedeltaExprMonad(
                timedelta,
                ["DATETIME_DIFF", monad.getsql()[0], other.getsql()[0]],
                nullable=monad.nullable or other.nullable,
            )
        throw(
            TypeError, _binop_errmsg % (type2str(monad.type), type2str(other.type), "-")
...
def make_string_binop(op: str, sqlop: str) -> Callable[[Monad, Monad], StringExprMonad]:
...
class StringMixin(MonadMixin):
    ...
    __add__: Callable[[Monad, Monad], StringExprMonad] = make_string_binop(
        "+", "CONCAT"
    )

    def __getitem__(
        monad, index: NumericMixin | slice
        ...
        def param_to_const(
            monad: Monad | NumericMixin, is_start: bool = True
        ) -> Monad | NumericMixin:
            if isinstance(monad, ParamMonad):
                ...
                if key in root_translator.fixed_param_values:
                    ...
                    assert root_translator.vars is not None
                ...
                return ConstMonad.new(cast(Constant | tuple, index_value))
            return monad
        ...
        if isinstance(index, ListMonad):
            ...
            if start_value == 0 and stop_value == -1:
                return monad
            if (
                isinstance(monad, StringConstMonad)
        ...
        index = cast(NumericMixin, param_to_const(index))
    ...
    call_upper: ClassVar[Callable[[Monad], StringExprMonad]] = make_string_func("UPPER")
    call_lower: ClassVar[Callable[[Monad], StringExprMonad]] = make_string_func("LOWER")
    ...
    def _like(
        ...
        if (
            not_like
            and monad.nullable
            and not isinstance(monad, AttrMonad)
        ...
        if (
            not_like
            and monad.nullable
            and (isinstance(monad, AttrMonad) or translator.dialect == "Oracle")
        ):
            result_sql = ["OR", result_sql, ["IS_NULL", sql]]
    ...
    def call_strip(monad, chars: NumericConstMonad | None = None) -> StringExprMonad:
        return monad.strip(chars, "TRIM")

    def call_lstrip(monad, chars: StringConstMonad | None = None) -> StringExprMonad:
...
class JsonMixin(object):
    ...
    def __getitem__(
        monad, key: EllipsisMonad | NumericMixin | slice | StringMixin
    ) -> JsonItemMonad:
    ...
    def cast_from_json(monad, type: type) -> Self | ExprMonad:
...
class ArrayMixin(MonadMixin):
    ...
    def _index(
        ...
        if isinstance(index, NumericConstMonad):
            ...
            value = cast(Numeric, index_sql[1])
            if value >= 0:
...
class ObjectMixin(MonadMixin):
    ...
    def requires_distinct(monad, joined: bool = False) -> bool:
        attr = getattr(monad, "attr")
        return attr.reverse.is_collection or monad.parent.requires_distinct(
            joined
        )  # parent ???
...
class ObjectIterMonad(ObjectMixin, Monad):
    ...
    def requires_distinct(monad, joined: bool = False) -> bool:
        return (
            monad.tableref.name_path != monad.translator.tree.generators[-1].target.id
        )
...
class AttrMonad(Monad):
    ...
    def new(
        parent: ObjectMixin,
        attr: Attribute[AttrValue | Entity],
        *args,
        **kwargs,
    ) -> AttrMonad:
...
class ObjectAttrMonad(ObjectMixin, AttrMonad):
    tableref: TableRef | JoinedTableRef | _typeshed.MaybeNone
    ...
    def __init__(
        ...
        if monad.tableref is None:
            ...
            monad.tableref = parent_sqlquery.add_tableref(
...
class ParamMonad(Monad):
    ...
    @staticmethod
    def new(t: PyType | EntityMeta, paramkey: ParamKey) -> ParamMonad:
        t = normalize_type(t)
        if t in numeric_types:
...
class ObjectParamMonad(ObjectMixin, ParamMonad):
    ...
    def __init__(
        ...
        if monad.translator.database is not entity._database_:
            assert monad.translator.database is entity._database_, (
                paramkey,
                monad.translator.database,
                entity._database_,
            )
        varkey, i, j = paramkey
        assert j is None
    ...
    def getsql(
        ...
        return [
            ...
            for param, converter in zip(monad.params, entity._pk_converters_)
        ]
...
class ArrayParamMonad(ArrayMixin, ParamMonad):
    ...
    def __init__(
        monad, t: type[Array], paramkey: ParamKey, list_monad: ListMonad | None = None
    ) -> None:
    ...
    def contains(monad, key: Monad, not_in: bool = False) -> BoolExprMonad:
        if key.type is monad.type.item_type:
            assert monad.list_monad is not None
...
class ExprMonad(Monad):
    ...
    @staticmethod
    def new(t: _ExprMonadType, sql: _Sql, nullable: bool = True) -> ExprMonad:
        if t in numeric_types:
    ...
    def __init__(monad, type: PyType, sql: _Sql, nullable: bool = True) -> None:
        Monad.__init__(monad, type, nullable=nullable)
        monad.sql = sql
...
class JsonItemMonad(JsonMixin, Monad):
    ...
    def cast_from_json(monad, type):
        ...
        if type is NoneType:
            return ExprMonad.new(Json, sql)
        return ExprMonad.new(cast(_ExprMonadType, type), sql)
...
class ConstMonad(Monad, Generic[_CT]):
    ...
    def __init__(monad, value: _CT) -> None:
        value_type, value = normalize(value)
        Monad.__init__(monad, value_type, nullable=value_type is NoneType)
        monad.value = value

    def getsql(monad, sqlquery: SqlQuery | None = None) -> list[Sequence[str | _CT]]:
...
class BoolMonad(Monad):
    def __init__(monad, nullable: bool | None = True) -> None:
...
class CmpMonad(BoolMonad):
    ...
    def __init__(monad, op: str, left: Monad, right: Monad) -> None:
        if op == "<>":
            op = "!="
    ...
    def getsql(monad, sqlquery: SqlQuery | None = None) -> list[Sequence[str | list]]:
        op = monad.op
        if (
            monad.left.type is NoneType and monad.right.type is NoneType
        ):  # in hybrid methods
    ...
    def __init__(
        ...
        for operand in operands:
            if operand.type is not bool:
                ...
                isinstance(operand, LogicalBinOpMonad) and monad.binop == operand.binop
            ):
...
class HybridFuncMonad(Monad):
    ...
    def __call__(monad, *args, **kwargs) -> Monad:
        ...
        try:
            ...
            throw(
                TranslationError,
                "%s(...) is too complex to decompile" % monad.func_name,
            )
...
class HybridMethodMonad(HybridFuncMonad):
    def __init__(monad, parent: ObjectIterMonad, attrname: str, func: Callable) -> None:
        entity = parent.type
        assert isinstance(entity, EntityMeta)
...
class FuncIsinstanceMonad(FuncMonad):
    func: ClassVar[Callable[..., bool]] = isinstance
...
class FuncBufferMonad(FuncMonad):
    func: ClassVar[Callable[..., bytes]] = buffer
...
class FuncBoolMonad(FuncMonad):
    func: ClassVar[Callable[..., bool]] = bool
...
class FuncDateMonad(FuncMonad):
    func: ClassVar[Callable[..., date]] = date
...
class FuncTimeMonad(FuncMonad):
    func: ClassVar[Callable[..., time]] = time
...
class FuncTimedeltaMonad(FuncMonad):
    func: ClassVar[Callable[..., timedelta]] = timedelta
...
class FuncDatetimeMonad(FuncDateMonad):
    func: ClassVar[Callable[..., datetime]] = datetime
    ...
    def call(
        ...
        return cast(DatetimeConstMonad, ConstMonad.new(value))
...
class FuncBetweenMonad(FuncMonad):
    func: ClassVar[Callable[..., bool]] = between
...
class FuncConcatMonad(FuncMonad):
    func: ClassVar[Callable[..., str]] = concat
    ...
    def call(monad, *args) -> StringExprMonad:
        ...
        return ExprMonad.new(
            str, cast(_Sql, result_ast), nullable=any(arg.nullable for arg in args)
        )


class FuncLenMonad(FuncMonad):
    func: ClassVar[Callable[..., int]] = len

    def call(monad, x: Monad) -> NumericExprMonad:
        return x.len()


class FuncGetattrMonad(FuncMonad):
    func: ClassVar[Callable] = getattr
    ...
    def call(
        ...
        if isinstance(name_monad, ConstMonad):
            ...
            if key in translator.fixed_param_values:
                ...
                assert translator.vars is not None
...
class FuncRawSQLMonad(FuncMonad):
    func: ClassVar[Callable[..., RawSQL]] = raw_sql
...
class FuncGroupConcatMonad(FuncMonad):
    ...
    ] = (utils.group_concat, core.group_concat)
...
class FuncCoalesceMonad(FuncMonad):
    func: ClassVar[Callable[[Sequence[OptAttrValue]], OptAttrValue]] = coalesce
    ...
    def call(monad, *args) -> ExprMonad:
        ...
        return ExprMonad.new(
            cast(_ExprMonadType, t),
            cast(_Sql, sql),
            nullable=all(arg.nullable for arg in args),
        )
...
class FuncMinMonad(FuncMonad):
    func: ClassVar[tuple[Callable, Callable]] = (min, core.min)
...
class FuncMaxMonad(FuncMonad):
    func: ClassVar[tuple[Callable, Callable]] = max, core.max
...
def minmax(monad: FuncMaxMonad | FuncMinMonad, sqlop: str, *args) -> ExprMonad:
    ...
    sql = cast(_Sql, [sqlop, None] + [arg.getsql()[0] for arg in args])
    return ExprMonad.new(
        cast(_ExprMonadType, t), sql, nullable=any(arg.nullable for arg in args)
    )


class FuncSelectMonad(FuncMonad):
    func: ClassVar[Callable[..., Query]] = core.select
...
class FuncExistsMonad(FuncMonad):
    func: ClassVar[Callable[..., bool]] = core.exists
...
class FuncDescMonad(FuncMonad):
    func: ClassVar[Callable[..., DescWrapper | Attribute | AttrValue]] = core.desc
...
class DescMonad(Monad):
    ...
    def getsql(monad, sqlquery: SqlQuery | None = None) -> list[list[list[str] | str]]:
...
class FuncRandomMonad(FuncMonad):
    func: ClassVar[Callable[[], float]] = random
...
class AttrSetMonad(SetMixin, Monad):
    ...
    def cmp(monad, op: str, monad2: Monad):
        if type(monad2.type) is SetType and are_comparable_types(
            monad.type.item_type, monad2.type.item_type
        ):
            pass
    ...
    def contains(monad, item: Monad, not_in: bool = False) -> BoolExprMonad:
        ...
        if not translator.hint_join:
            ...
            cast(list, from_ast[-1])[-1] = sqland([from_ast[-1][-1]] + conditions)
            expr_ast = sqland([["IS_NULL", expr] for expr in expr_list])
            return BoolExprMonad(expr_ast, nullable=False)

    def getattr(monad, name: str) -> Monad:  # type: ignore
        try:
    ...
    def requires_distinct(monad, joined: bool = False, for_count: bool = False) -> bool:
    ...
    def count(monad, distinct: NumericConstMonad | None = None) -> ExprMonad:
        translator = monad.translator
        is_distinct = distinct_from_monad(
            distinct,
            monad.requires_distinct(joined=translator.hint_join, for_count=True),
        )
        ...
        assert monad.tableref is not None
        if not is_distinct and monad.tableref.name_path != translator.optimize:
            ...
            assert isinstance(translator, SQLiteTranslator)
            if not is_distinct:
                alias, pk_columns = monad.tableref.make_join(pk_only=True)
                make_aggr = lambda expr_list: [
                    "COUNT",
    ...
    def aggregate(
        ...
        is_distinct = distinct_from_monad(
            distinct, default=monad.forced_distinct and func_name in ("SUM", "AVG")
        )
        translator = monad.translator
        ...
        def make_aggr(expr_list):
            result = [func_name, is_distinct] + expr_list
            if sep is not None:
                assert func_name == "GROUP_CONCAT"
                result.append(["VALUE", sep])
            return result

        # make_aggr = lambda expr_list: [ func_name, is_distinct ] + expr_list
        ...
        if func_name == "AVG":
            ...
            result_type = cast(_ExprMonadType, item_type)
        assert monad.tableref is not None
        translator.aggregated_subquery_paths.add(monad.tableref.name_path)
    ...
    def make_tableref(
        ...
        if attr.reverse:
            ...
            monad.tableref = sqlquery.get_tableref(name_path) or sqlquery.add_tableref(
                name_path, parent_tableref, attr
            )
        else:
    ...
    def _aggregated_scalar_subselect(
        ...
        if translator.optimize == monad.tableref.name_path:
            ...
            if not translator.from_optimized:
                assert monad.sqlquery is not None
                from_ast = monad.sqlquery.from_ast[1:]
                assert sqlquery.outer_conditions
                from_ast[0] = cast(list, from_ast[0]) + [
                    sqland(sqlquery.outer_conditions)
                ]
    ...
    def _subselect(
        ...
        if sqlquery is not translator.sqlquery and extract_outer_conditions:
            assert isinstance(sqlquery.from_ast[1], list)
            outer_cond = sqlquery.from_ast[1].pop()
            assert isinstance(outer_cond, list)
            if outer_cond[0] == "AND":
    ...
    __add__: Callable[[Monad, Monad], NumericSetExprMonad] = make_attrset_binop(
        "+", "ADD"
    )
    __sub__: Callable[[Monad, Monad], NumericSetExprMonad] = make_attrset_binop(
        "-", "SUB"
    )
    __mul__: Callable[[Monad, Monad], NumericSetExprMonad] = make_attrset_binop(
        "*", "MUL"
    )
    __truediv__: Callable[[Monad, Monad], NumericSetExprMonad] = make_attrset_binop(
        "/", "DIV"
    )
    __floordiv__: Callable[[Monad, Monad], NumericSetExprMonad] = make_attrset_binop(
        "//", "FLOORDIV"
    )
...
class NumericSetExprMonad(SetMixin, Monad):
    ...
    def aggregate(
        ...
        aggr_ast = [func_name, is_distinct, expr]
        if func_name == "GROUP_CONCAT":
            if sep is not None:
                aggr_ast.append(["VALUE", sep])
        if translator.optimize != monad.tableref.name_path:
    ...
    def getsql(monad, sqlquery: SqlQuery | None = None) -> list[list[str]]:
        ...
        if left_path.startswith(right_path):
            ...
            throw(
                TranslationError,
                "Cartesian product detected in %s" % ast2src(monad.node),
            )
        monad.tableref = tableref
        return [[monad.sqlop, left_expr, right_expr]]

    __add__: Callable[[Monad, Monad], NumericSetExprMonad] = make_numericset_binop(
        "+", "ADD"
    )
    __sub__: Callable[[Monad, Monad], NumericSetExprMonad] = make_numericset_binop(
        "-", "SUB"
    )
    __mul__: Callable[[Monad, Monad], NumericSetExprMonad] = make_numericset_binop(
        "*", "MUL"
    )
    __truediv__: Callable[[Monad, Monad], NumericSetExprMonad] = make_numericset_binop(
        "/", "DIV"
    )
    __floordiv__: Callable[[Monad, Monad], NumericSetExprMonad] = make_numericset_binop(
        "//", "FLOORDIV"
    )
...
class QuerySetMonad(SetMixin, Monad):
    ...
    def to_single_cell_value(monad) -> ExprMonad:
        return ExprMonad.new(
            cast(_ExprMonadType, monad.item_type), cast(_Sql, monad.getsql()[0])
        )
    ...
    def contains(monad, item: Monad, not_in: bool = False) -> BoolExprMonad:
        ...
        if translator.hint_join and len(sub.sqlquery.from_ast[1]) == 3:
            ...
            for i, column_ast in enumerate(select_ast):
                assert isinstance(select_ast, list) and isinstance(column_ast, list)
                if not i:
                    continue  # 'ALL'
            ...
            if len(item_columns) == 1:
                subquery_ast = sub.construct_subquery_ast(
                    monad.limit, monad.offset, distinct=False, is_not_null_checks=not_in
                )
                ...
                if ambiguous_names:
                    ...
                    for i, expr_ast in enumerate(select_ast):
                        assert isinstance(select_ast, list) and isinstance(
                            expr_ast, list
                        )
                        if i > 0:
                            if expr_ast[0] == "AS":
    ...
    def nonzero(monad) -> BoolExprMonad:
        ...
        if not isinstance(expr_monad, ObjectIterMonad):
            ...
            assert isinstance(subquery_ast[3], list)
    ...
    def negate(monad) -> BoolExprMonad:
        ...
        assert sql[0] == "EXISTS"
        return BoolExprMonad(["NOT_EXISTS"] + list(sql[1:]), nullable=False)

    def count(
        monad, distinct: NumericConstMonad | BoolExprMonad | None = None
    ) -> ExprMonad:
        is_distinct = distinct_from_monad(distinct)
        translator = monad.translator
        sub = monad.subtranslator
        ...
        if isinstance(expr_type, (tuple, EntityMeta)):
            if not sub.distinct and not is_distinct:
                select_ast = ["AGGREGATES", ["COUNT", None]]
            elif len(sub.expr_columns) == 1:
                select_ast = [
                    "AGGREGATES",
                    ["COUNT", True if is_distinct is None else is_distinct]
                    + sub.expr_columns,
                ]
            elif translator.dialect == "Oracle":
                sql_ast = [
                ...
                if translator.sqlite_version < (3, 6, 21):
                    ...
                    sql_ast = [
                        ...
                        [
                            ...
                            [
                                "COUNT",
                                True if is_distinct is None else is_distinct,
                                ["COLUMN", alias, "ROWID"],
                            ],
                        ],
                        from_ast,
                    ...
                    sql_ast = [
                        ...
                        [
                            ...
                            [
                                ...
                                "SELECT",
                                [
                                    ["DISTINCT" if is_distinct is not False else "ALL"]
                                    + sub.expr_columns,
                                    from_ast,
                                    where_ast,
                                ],
    ...
    def aggregate(
        ...
        aggr_ast = [func_name, is_distinct, sub.expr_columns[0]]
        if func_name == "GROUP_CONCAT":
            if sep is not None:
                aggr_ast.append(["VALUE", sep])
        select_ast = ["AGGREGATES", aggr_ast]
...
def find_or_create_having_ast(sections: list) -> list:
    ...
    for i, section in enumerate(sections):
        ...
        if section_name == "GROUP_BY":
            ...
            return section
    having_ast = ["HAVING"]
    assert isinstance(groupby_offset, int)
