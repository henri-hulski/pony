...
class OraTable(Table):
    def get_objects_to_create(table, created_tables=None):
        result = cast(list, Table.get_objects_to_create(table, created_tables))
...
class OraSequence(DBObject):
    name: TableName
    table: OraTable
    typename: ClassVar[str] = "Sequence"

    def __init__(sequence, table: OraTable, name: TableName | None = None):
        ...
        if name is not None:
            ...
            sequence.name = cast(tuple[str], tuple(table_name[:-1])) + (
                table_name[0] + "_SEQ",
            )
    ...
    def exists(
        ...
        row = cast(Sequence[str] | None, cursor.fetchone())
...
class OraColumn(Column):
    auto_template: ClassVar[str | None] = None
...
class OraTranslator(sqltranslation.SQLTranslator):
    ...
    json_path_wildcard_syntax: ClassVar[bool] = True
    json_values_are_comparable: ClassVar[bool] = False
    NoneMonad: type[OraNoneMonad] = OraNoneMonad
    ConstMonad: type[OraConstMonad] = OraConstMonad
...
class OraBuilder(SQLBuilder):
    ...
    def DATE_DIFF(builder, expr1, expr2) -> tuple[Any, str, Any]:
    ...
    def DATETIME_DIFF(builder, expr1, expr2) -> tuple[Any, str, Any]:
        return builder(expr1), " - ", builder(expr2)
    ...
    def JSON_QUERY(builder, expr, path) -> tuple[str, Any, str, Param | Value, str]:
    ...
    def GROUP_CONCAT(builder, distinct, expr, sep=None):
        ...
        if distinct and cast(tuple[int, ...], builder.provider.server_version) >= (19,):
            distinct = "DISTINCT "
        else:
            distinct = ""
...
class OraStrConverter(dbapiprovider.StrConverter):
    ...
    def sql2py(converter, val):
        if isinstance(val, cx_Oracle.LOB):
            val = cast(cx_Oracle.LOB, val).read()
        return val
...
class OraBlobConverter(dbapiprovider.BlobConverter):
    def sql2py(converter, val):
        return buffer(cast(cx_Oracle.LOB, val).read())
    ...
    def sql2py(converter, val):
        if isinstance(val, timedelta):
            ...
            throw(
                ...
                % (
                    "for attribute %s" % converter.attr if converter.attr else "",
                    type(val),
                ),
            )
...
class OraProvider(DBAPIProvider):
    ...
    max_name_len: int = 30
    table_if_not_exists_syntax: bool = False
    index_if_not_exists_syntax: bool = False
    varchar_default_max_len: int | None = 1000
    uint64_support: bool = True

    dbapi_module: ModuleType = cx_Oracle
    dbschema_cls: type[OraSchema] = OraSchema
    translator_cls: type[OraTranslator] = OraTranslator
    sqlbuilder_cls: type[OraBuilder] = OraBuilder

    name_before_table: str = "owner"
    ...
    def inspect_connection(provider, connection):
        ...
        provider.server_version = get_version_tuple(cast(list, cursor.fetchone())[0])
        cursor.execute("SELECT sys_context( 'userenv', 'current_schema' ) FROM DUAL")
        provider.default_schema_name = cast(list, cursor.fetchone())[0]
    ...
    def execute(provider, cursor, sql, arguments=None, returning_id=False):
        if type(arguments) is list:
            ...
            if returning_id:
                ...
                arguments["new_id"] = var  # type: ignore
    ...
    def table_has_data(provider, connection, table_name):
        cursor = connection.cursor()
        cursor.execute(
            "SELECT 1 FROM %s WHERE ROWNUM = 1" % provider.quote_name(table_name)
        )
...
class OraPool(Pool):
    ...
    def __init__(pool, **kwargs):
        pool.kwargs: dict[str, Any] = kwargs
        pool.cx_pool: cx_Oracle.SessionPool = cx_Oracle.SessionPool(**kwargs)
        pool.pid: int = os.getpid()
