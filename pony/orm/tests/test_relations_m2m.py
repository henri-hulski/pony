...
class TestManyToManyNonComposite(unittest.TestCase):
    ...
    def test_9(self):
        with db_session:
            ...
            assert g is not None and s1 is not None and s2 is not None
            g.subjects.clear()
    ...
    def test_10(self):
        with db_session:
            ...
            s1 = Subject.get(name='Subj1')
            assert g2 is not None and s1 is not None
            g2.subjects.add(s1)
            g2.subjects.clear()
            last_sql = db.last_sql
    ...
    def test_14(self):
        g1 = Group[101]
        s1 = Subject['Subj1']
        assert g1._vals_ is not None and s1._vals_ is not None
