...
def wrap_dbapi_exceptions(
    ...
    **kwargs: _P.kwargs,
    ...
    try:
        ...
        if should_retry:
            e.should_retry = True  # type: ignore
        raise
...
class DBAPIProvider(object):
    ...
    quote_char: str = '"'
    max_params_count: int = 999
    max_name_len: int = 128
    table_if_not_exists_syntax: bool = True
    index_if_not_exists_syntax: bool = True
    max_time_precision: int = 6
    default_time_precision: int = 6
    uint64_support: bool = False

    # SQLite and PostgreSQL does not limit varchar max length.
    varchar_default_max_len: int | None = None

    dialect: str | _typeshed.MaybeNone = None
    dbapi_module: ModuleType | _typeshed.MaybeNone = None
    dbschema_cls: type[DBSchema] | _typeshed.MaybeNone = None
    translator_cls: type[SQLTranslator] | _typeshed.MaybeNone = None
    sqlbuilder_cls: type[SQLBuilder] | _typeshed.MaybeNone = None
    array_converter_cls: type[ArrayConverter] | _typeshed.MaybeNone = None

    name_before_table: str = "schema_name"
    default_schema_name: str | _typeshed.MaybeNone = None

    fk_types: dict[str, str] = {"SERIAL": "INTEGER", "BIGSERIAL": "BIGINT"}
    ...
    def get_default_index_name(
        ...
        if is_pk:
            ...
            index_name = template % dict(
                tname=provider.base_name(table_name),
    ...
    def get_default_fk_name(
        ...
        fk_name = "fk_%s__%s" % (
            provider.base_name(child_table_name),
    ...
    @wrap_dbapi_exceptions
    def connect(provider) -> tuple[DbConnection, bool]:
        ...
        if isinstance(py_type, type):
            ...
            if issubclass(py_type, Array):
                ...
                if converter_cls is None:
                    throw(
                        NotImplementedError,
                        "Array type is not supported for %r" % provider.dialect,
                    )
                assert converter_cls is not None and issubclass(
                    converter_cls, ArrayConverter
                )
                return converter_cls
    ...
    def get_converter_by_py_type(provider, py_type: _ExtPyType) -> "Converter":
...
class Pool(localbase):
    args: tuple
    con: DbConnection | _typeshed.MaybeNone
    dbapi_module: DBAPIProvider
    kwargs: dict[str, Any]
    pid: int | _typeshed.MaybeNone

    forked_connections: ClassVar[list[tuple[DbConnection, int]]] = []

    def __init__(
        pool, dbapi_module: DBAPIProvider, *args, **kwargs
...
class Converter(Generic[_OAT_co]):
    ...
    EQ: ClassVar[LiteralString] = "EQ"
    NE: ClassVar[LiteralString] = "NE"
    optimistic: ClassVar[bool] = True

    def __deepcopy__(converter, memo: dict[int, object]) -> Self:
    ...
    def dbval2val(
        self, dbval: OptAttrValue | Entity, obj: Entity | _typeshed.MaybeNone = None
    ) -> Any:
        return dbval
...
class StrConverter(Converter):
    ...
    def validate(
        converter, val: str, obj: Entity | None = None
    ) -> str | _typeshed.MaybeNone:
...
class IntConverter(Converter):
    ...
    unsigned_types: dict[int | None, str] | None = None
    py_type: type[int]
    ...
    def init(converter, kwargs: dict[str, Any]) -> None:
        ...
        if size == 64 and unsigned and not converter.provider.uint64_support:
            throw(
                ...
                % (attr, converter.provider.dialect),
            )
    ...
    def validate(
        ...
        if isinstance(val, int_types):
            ...
            val = cast(SupportsIndex, val).__index__()
...
class RealConverter(Converter):
    ...
    default_tolerance: ClassVar[float] = 1e-14
    optimistic = False
    ...
    def validate(
        ...
        try:
            ...
            throw(
                TypeError, "Invalid value for attribute %s: %r" % (converter.attr, val)
            )
    ...
    def sql2py(converter, val: float | str | SupportsFloat | SupportsIndex) -> float:
        return float(val)
...
class BlobConverter(Converter):
    ...
    def validate(converter, val: bytes | str, obj: Entity | None = None) -> bytes:
        ...
        throw(
            TypeError,
            "Attribute %r: expected type is 'buffer'. Got: %r"
            % (converter.attr, type(val)),
        )
...
class DateConverter(Converter):
    ...
    def validate(converter, val: date | str, obj: Entity | None = None) -> date:
        ...
        throw(
            TypeError,
            "Attribute %r: expected type is 'date'. Got: %r" % (converter.attr, val),
        )
...
class TimeConverter(ConverterWithMicroseconds):
    ...
    sql_type_name: LiteralString = "TIME"
    ...
    def validate(converter, val: time | str, obj: Entity | None = None) -> time:
        if isinstance(val, time):
            ...
            throw(
                TypeError,
                "Attribute %r: expected type is 'time'. Got: %r"
                % (converter.attr, val),
            )
...
class TimedeltaConverter(ConverterWithMicroseconds):
    ...
    sql_type_name: LiteralString = "INTERVAL"
...
class DatetimeConverter(ConverterWithMicroseconds):
    ...
    sql_type_name: LiteralString = "DATETIME"

    def validate(converter, val: datetime | str, obj: Entity | None = None) -> datetime:
        if isinstance(val, datetime):
            pass
        elif isinstance(val, str):
            ...
            throw(
                TypeError,
                "Attribute %r: expected type is 'datetime'. Got: %r"
                % (converter.attr, val),
            )
        mcs = converter.round_microseconds_to_precision(
            val.microsecond, converter.precision
...
class JsonConverter(Converter):
    ...
    class JsonEncoder(json.JSONEncoder):
        def default(converter, obj: Json) -> dict[str, str] | list[str]:  # type: ignore
            if isinstance(obj, Json):
                return obj.wrapped
            return json.JSONEncoder.default(converter, obj)

    def validate(converter, val: Any, obj: Entity | None = None) -> TrackedValue:
        if obj is None or converter.attr is None:
            return val
        if (
            isinstance(val, TrackedValue)
            and val.obj_ref is not None
            and val.obj_ref() is obj
            and val.attr is converter.attr
        ):
            return val
        return TrackedValue.make(obj, converter.attr, val)

    def val2dbval(converter, val: Any, obj: Entity | None = None) -> str:
    ...
    def dbval2val(
        ...
        if isinstance(dbval, (int, bool, float, type(None))):
            return dbval
        val = json.loads(cast(str | bytes, dbval))
        if obj is None:
            return val
        return TrackedValue.make(obj, converter.attr, val)
...
class ArrayConverter(Converter):
        ...
        for i, v in enumerate(items):
            if not isinstance(v, item_type):
                if hasattr(v, "__index__"):
                    items[i] = cast(SupportsIndex, v).__index__()
                else:
                    throw(
                        TypeError,
    ...
    def val2dbval(
        ...
        assert converter.attr is not None
