...
from threading import RLock, current_thread, _MainThread  # type: ignore[attr-defined]
...
_PkValue: TypeAlias = (
    "_UniqueAttrValue | Entity | tuple[_UniqueAttrValue | Entity, ...]"
)
...
QueryVars: TypeAlias = dict[
    VarKey, "Query | QueryResult | QueryResultIterator | SetIterator | EntityMeta"
]
...
_CachedSql: TypeAlias = "tuple[str, Adapter, AttrOffsets | None]"
...
class DbConnection(Protocol):
    autocommit: bool
    server_version: int | tuple[int, ...] | _typeshed.MaybeNone
...
class PrefetchContext(object):
    ...
    def get_frozen_attrs_to_prefetch(self, entity: EntityMeta) -> frozenset[Attribute]:
        attrs_to_prefetch = self.attrs_to_prefetch_dict.get(entity, set())
        ...
        assert isinstance(attrs_to_prefetch, frozenset)
        return attrs_to_prefetch

    def get_relations_to_prefetch(self, entity: EntityMeta) -> tuple[Attribute, ...]:
        ...
        if result is None:
            ...
            result = tuple(
                ...
                and (
                    ...
                    or attr.py_type in self.entities_to_prefetch
...
class DBSessionContextManager(object):
    ...
    def __init__(
        ...
        retry_exceptions: Iterable[type[Exception]] | Callable = (TransactionError,),
        allowed_exceptions: Iterable[type[Exception]] | Callable = (),
    ...
    def __call__(db_session, *args, **kwargs):
        ...
        if (
            ...
            or hasattr(inspect, "iscoroutinefunction")
    ...
    def _wrap_coroutine_or_generator_function(
        ...
        def interact(iterator, input=None, exc_info=None):
            ...
            throw_: Any = getattr(iterator, "throw", None)
            if throw_ is None:
                reraise(*exc_info)
        ...
        if hasattr(types, "coroutine"):
            new_gen_func = types.coroutine(new_gen_func)  # type: ignore[attr-defined]
        return new_gen_func


db_session: DBSessionContextManager = DBSessionContextManager()
...
class SQLDebuggingContextManager(object):
    ...
    def __call__(self, *args, **kwargs) -> type[Self] | Callable:
    ...
    def _wrap_function(self, func: Callable[_P, _T]) -> Callable[_P, _T]:
        def new_func(func, *args, **kwargs):
            self.__enter__()
            try:
                return func(*args, **kwargs)
    ...
    def _wrap_generator_function(self, gen_func: Callable[_P, _T]) -> Callable[_P, _T]:
        def interact(iterator, input=None, exc_info=None):
            if exc_info is None:
                return next(iterator) if input is None else iterator.send(input)
...
sql_debugging: SQLDebuggingContextManager = SQLDebuggingContextManager()


def throw_db_session_is_over(action: str, obj: Entity, attr: Attribute | None = None):
...
@decorator
def db_decorator(func: Callable[_P, _T], *args, **kwargs) -> _T:
...
class OnConnectDecorator(object):
    ...
    def __call__(self, func=None, provider=None) -> OnConnectDecorator:
        if isinstance(func, types.FunctionType):
            self.database._on_connect_funcs.append((func, provider or self.provider))
        if not provider and isinstance(func, str):
            provider = func
        OnConnectDecorator.check_provider(provider)
...
class Database(object):
    ...
    priority: int
    provider: DBAPIProvider | _typeshed.MaybeNone
    provider_name: str | None
    schema: DBSchema | _typeshed.MaybeNone
    Entity: type[_Entity]
    ...
    @property
    def last_sql(database) -> str | _typeshed.MaybeNone:
    ...
    def insert(
        ...
        **kwargs,
        ...
        arguments = tuple(
            adapter(value) for value in kwargs.values()
        ...
        for entity in entities:
            ...
            for attr in entity._new_attrs_:
                if attr.is_collection:
                    ...
                    reverse = attr.__class__.reverse
                    ...
                    if not attr.reverse and attr.default is not None:
                        assert not isinstance(attr, Set)
                        assert len(attr.converters) == 1
                        if not callable(attr.default):
                            default = cast(AttrValue, attr.validate(attr.default))
    ...
    def drop_table(
        ...
        database._drop_tables(
            [table_name], if_exists, with_all_data, try_normalized=True
        )
        ...
        if isinstance(table_name, EntityMeta):
            ...
            for component in table_name:
                if not isinstance(component, str):
                    throw(
                        TypeError, "Invalid table name component: {}".format(component)
                    )
        elif isinstance(table_name, str):
            table_name = table_name[:]  # table_name = templating.plainstr(table_name)
        else:
            throw(TypeError, "Invalid table name: {}".format(table_name))
        assert isinstance(table_name, (str, tuple))
        return table_name
            ...
            if provider.table_exists(connection, table_name):
                ...
                if try_normalized:
                    if isinstance(table_name, str):
                        ...
                        normalized_table_name = schema_name, provider.normalize_name(
                            base_name
                        )
    ...
    def set_perms_for(database, *entities):
        ...
        local.perms_context = database, entity_set  # type: ignore[attr-defined]
        try:
            yield
        finally:
    ...
    def _get_schema_json(database):
        schema_json = json.dumps(
            database._get_schema_dict(), default=basic_converter, sort_keys=True
        )
            ...
            for name, val in diff.items():
                if name not in ("class", "_pk_", "_id_", "_status_"):
                    ...
                    if (
                        ...
                        if removed:
                            observer("remove", obj, {name: removed})  # type: ignore
                            collection.remove(removed)
                        if added:
                            observer("add", obj, {name: added})  # type: ignore
...
def perm(*args, **kwargs):
    ...
    database, entities = local.perms_context  # type: ignore
...
def has_perm(user, perm, x):
    ...
    perm_cache = cache.perm_cache[user][perm]  # type: ignore[union-attr]
...
class QueryStat(object):
    cache_count: int
    db_count: int
    max_time: float | _typeshed.MaybeNone
    min_time: float | _typeshed.MaybeNone
    sql: str | None
    sum_time: float | _typeshed.MaybeNone
...
class SessionCache(object):
    collection_statistics: dict[Set, int] | _typeshed.MaybeNone
    connection: DbConnection | _typeshed.MaybeNone
    database: Database
    db_session: DBSessionContextManager | None
    ...
    for_update: set[Entity] | _typeshed.MaybeNone
    immediate: bool
    in_transaction: bool
    indexes: (
        dict[
    ...
    is_alive: bool
    max_id_cache: dict[Attribute, int] | _typeshed.MaybeNone
    modified: bool
    modified_collections: dict[Set, set[Entity]] | _typeshed.MaybeNone
    noflush_counter: int
    num: int
    objects: set[Entity] | _typeshed.MaybeNone
    objects_to_save: list[Entity | None] | _typeshed.MaybeNone
    query_results: dict[QueryKey, list[Entity]] | _typeshed.MaybeNone
    saved_fk_state: bool | None
    saved_objects: list[tuple[Entity, LiteralString]] | _typeshed.MaybeNone
    seeds: dict[tuple[Attribute, ...], set[Entity]] | _typeshed.MaybeNone
    ...
    def prepare_connection_for_query_execution(cache) -> DbConnection:
        ...
        if db_session is not None and cache.db_session is None:
            ...
            assert cache.db_session is db_session, (cache.db_session, db_session)
        connection = cache.connection
    ...
    def close(cache, rollback: bool = True) -> None:
        ...
        try:
            ...
            if db_session and db_session.strict:
                ...
                cache.perm_cache = cache.user_roles_cache = cache.obj_labels_cache = (
                    None
                )
            else:
                for obj in cache.objects:
            ...
            cache.objects = cache.objects_to_save = cache.saved_objects = (
                cache.query_results
            ) = cache.indexes = cache.seeds = cache.for_update = cache.max_id_cache = (
                cache.modified_collections
            ) = cache.collection_statistics = cache.dbvals_deduplication_cache = None
    ...
    def _calc_modified_m2m(cache) -> dict[Collection, tuple[set[Any], set[Any]]]:
        modified_m2m = {}
        for attr, objects in sorted(
            cache.modified_collections.items(),
    ...
    def update_simple_index(
        ...
        if old_val is not None:
            assert not isinstance(old_val, NotLoadedValueType)
    ...
    def update_composite_index(
        ...
        prev_vals: tuple[OptAttrValue, ...] | _typeshed.MaybeNone,
        new_vals: tuple[OptAttrValue, ...] | _typeshed.MaybeNone,
        undo: list[Any],
    ) -> None:
        assert prev_vals is not None or new_vals is not None
...
NOT_LOADED: NotLoadedValueType = NotLoadedValueType()
...
DEFAULT: DefaultValueType = DefaultValueType()
...
class Attribute(Generic[_OAET_co]):
    ...
    entity: EntityMeta | _typeshed.MaybeNone
    fk_name: str | None
    hidden: bool
    id: int
    index: str | None
    ...
    def __init__(
        ...
        **kwargs,
        ...
        attr.py_type = py_type  # type: ignore[attr-defined]
        attr.is_string = type(py_type) is type and issubclass(py_type, str)
        attr.type_has_empty_value = attr.is_string or hasattr(
            attr.py_type, "default_empty_value"
        )
        attr.is_collection = isinstance(attr, Collection)
        attr.is_relation = isinstance(
            attr.py_type, (EntityMeta, str, types.FunctionType)
        )
        ...
        if not attr.reverse:
            ...
            throw(
                TypeError,
                "Reverse option cannot be set for this type: %r" % attr.py_type,
            )
        ...
        if attr.column is not None:
            ...
            if not isinstance(attr.column, str):
                throw(
                    TypeError,
                    "Parameter 'column' must be a string. Got: %r" % attr.column,
                )
            ...
            if not isinstance(attr.columns, (tuple, list)):
                throw(
                    TypeError,
                    "Parameter 'columns' must be a list. Got: %r'" % attr.columns,
                )
    ...
    def _init_(attr, entity: EntityMeta, name: str) -> None:
        ...
        if "default" in attr.kwargs:
            ...
            attr.default = "" if attr.is_string else attr.py_type.default_empty_value()  # type: ignore[attr-defined]
        else:
            attr.default = None
        ...
        if attr.py_check is not None and not callable(attr.py_check):
            throw(
                TypeError,
                "'py_check' parameter of %s attribute should be callable" % attr,
            )
        ...
        if attr.is_volatile and attr.is_pk:
            throw(
                TypeError,
        ...
        if attr.interleave is not None:
            ...
            if attr.interleave not in (True, False):
                throw(
                    TypeError,
    ...
    def linked(attr) -> None:
        ...
        if attr.is_collection and not reverse.is_collection:
            if attr.fk_name is not None:
                throw(
                    TypeError,
                    "You should specify fk_name in %s instead of %s" % (reverse, attr),
                )
    ...
    def _get_entity(attr, obj: Entity | None, entity: EntityMeta | None) -> EntityMeta:
    ...
    def validate(
        ...
        if not reverse:
            ...
            assert not isinstance(val, Entity)
        ...
        if attr.py_check is not None and not attr.py_check(val):
            throw(
                ValueError,
                "Check for attribute %s failed. Value: %s" % (attr, truncate_repr(val)),
            )
        return val
    ...
    def parse_value(
        ...
        if not attr.reverse:
            ...
            if None in dbvals:
                ...
                dbval = attr.py_type._get_by_raw_pkval_(dbvals)
        return dbval

    def load(attr, obj: Entity) -> Any:
        cache = obj._session_cache_
        if cache is None or not cache.is_alive:
            throw_db_session_is_over("load attribute", obj, attr)
        if not attr.columns:
            reverse = attr.reverse
    ...
    def __set__(attr, obj, new_val, undo_funcs=None):
        cache = obj._session_cache_
        if cache is None or not cache.is_alive:
            throw_db_session_is_over("assign new value to", obj, attr)
        ...
        if attr.pk_offset is not None:
            ...
            if pkval is None:
                ...
                assert isinstance(pkval, EntityMeta)
    ...
    def db_set(
        ...
        if new_dbval is NOT_LOADED:
            ...
            assert not isinstance(new_dbval, NotLoadedValueType)
        ...
        if not reverse:
            ...
            assert isinstance(old_dbval, Entity) and isinstance(new_dbval, Entity)
            attr.db_update_reverse(obj, old_dbval, new_dbval)
        elif old_dbval not in (None, NOT_LOADED):
            assert isinstance(old_dbval, Entity)
            if not reverse.is_collection:
                if new_dbval is not NOT_LOADED:
    ...
    def get_columns(attr) -> list[str]:
        ...
        if not reverse:  # attr is not part of relationship
            ...
            def generate_columns():
                ...
                attr.col_paths = [
                    "-".join((attr.name, paths)) for paths in reverse_pk_col_paths
                ]
                attr.converters = []
    ...
    def describe(attr) -> str:
        ...
        options = []
        if attr.args:
            options.append(", ".join(map(str, attr.args)))
        if attr.auto:
...
class Required(Attribute[_AET_co]):
    ...
    def validate(
        ...
        if val == "" or (
            val is None and not (attr.auto or attr.is_volatile or attr.sql_default)
        ):
            if not from_db:
                throw(
                    ValueError,
...
class Discriminator(Required[_AT_co]):
    ...
    @staticmethod
    def create_default_attr(entity: EntityMeta) -> None:
    ...
    def process_entity_inheritance(attr, entity: EntityMeta) -> None:
    ...
    def validate(
        ...
        validated = Attribute.validate(attr, val, obj, entity)
        return cast(_AT_co, validated)
...
class Index(object):
    ...
    def __init__(index, *attrs, **options) -> None:
        index.entity = None
        index.attrs = list(attrs)  # type: ignore[attr-defined]
        index.is_pk = options.pop("is_pk", False)
        index.is_unique = options.pop("is_unique", True)
        assert not options

    def _init_(index, entity: EntityMeta) -> None:
        index.entity = entity
        attrs = index.attrs
        ...
        for i, attr in enumerate(attrs):
            if not isinstance(attr, Attribute):
                func_name = (
                    "PrimaryKey"
                    if index.is_pk
                    else "composite_key" if index.is_unique else "composite_index"
                )
                throw(
                    TypeError,
            ...
            key_type = (
                "primary key"
                if index.is_pk
                else "unique index" if index.is_unique else "index"
            )
            if attr.is_collection or (
                index.is_pk and not attr.is_required and not attr.auto
            ):
                throw(
                    TypeError,
...
class Collection(Attribute[_ET_co]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if attr.reverse_column is not None:
            if attr.reverse_columns is not None and attr.reverse_columns != [
                attr.reverse_column
            ]:
                throw(
                    TypeError,
    ...
    def load(attr, obj, items=None) -> Any:
...
class Set(Collection[_ET_co]):
    ...
    def validate(
        ...
        if val is DEFAULT:
            return set()
        reverse: Attribute = attr.reverse
        if val is None:
            throw(
                ValueError,
        ...
        assert not isinstance(val, DefaultValueType)
        if entity is not None:
            pass
    ...
    def prefetch_load_all(attr, objects: Iterable[Entity]) -> set[Entity]:
        ...
        reverse = attr.reverse
        rentity = reverse.entity
        # TODO: _get_raw_pkval_ is Attribute of Entity not EntityMeta
        objects = sorted(objects, key=entity._get_raw_pkval_)  # type: ignore[attr-defined]
    ...
    def load(attr, obj: Entity, items: Iterable[Entity] | None = None) -> SetData:
        cache = obj._session_cache_
        if cache is None or not cache.is_alive:
            throw_db_session_is_over("load collection", obj, attr)
    ...
    def construct_sql_m2m(
        ...
        select_list: list[str | list[str]] = ["ALL"]
        if not attr.symmetric:
    ...
    def reverse_add(
        attr, objects: Iterable[Entity], item: Entity, undo_funcs: list[Callable]
    ) -> None:
        undo = []
        ...
        for obj in objects:
            ...
            if setdata.count is not None:
                setdata.count += 1
            if in_removed:
                assert setdata.removed is not None
    ...
    def remove_m2m(attr, removed: set[tuple[Entity, Entity]]) -> None:
        ...
        if cached_sql is None:
            reverse = attr.reverse
            where_list: list[str | list] = ["WHERE"]
            if attr.symmetric:
...
class SetInstance(Generic[_ET_co]):
    ...
    def is_empty(wrapper) -> bool:
        ...
        if cached_sql is None:
            where_list: list[str | list[str | list]] = ["WHERE"]
            for i, (column, converter) in enumerate(
...
class Multiset(object):
    ...
    def __repr__(multiset):
        ...
        if cache is not None and cache.is_alive:
            ...
            size_str = ""
        return "<%s %r.%s%s>" % (
            multiset.__class__.__name__,
            multiset._obj_,
            ".".join(multiset._attrnames_),
            size_str,
        )
    ...
    def __str__(multiset):
        items_str = "{%s}" % ", ".join(
            "%r: %r" % pair for pair in sorted(multiset._items_.items())
        )
...
class EntityIter(object):
    def __init__(self, entity: EntityMeta) -> None:
        self.entity = entity
...
class EntityMeta(type):
    ...
    _database_: Database | _typeshed.MaybeNone
    _default_genexpr_: ast.GeneratorExp
    _delete_sql_cache_: dict[tuple[()], tuple[str, Callable]]
    _direct_bases_: list[EntityMeta]
    _discriminator_: AttrValue | _typeshed.MaybeNone
    ...
    _pk_columns_: list[str] | _typeshed.MaybeNone
    ...
    _table_: TableName | _typeshed.MaybeNone
    ...
    def __init__(
        ...
        direct_bases = [
            c
            for c in entity.__bases__
            if issubclass(c, Entity) and c.__name__ != "Entity"
        ]
        ...
        for name, attr in list(entity.__dict__.items()):
            ...
            if attr.entity is not None:
                throw(
                    ERDiagramError,
                    "Duplicate use of attribute %s in entity %s"
                    % (attr, entity.__name__),
                )
        ...
        interleave_attrs: list[Attribute] = []
        for attr in new_attrs:
            if attr.interleave is not None:
                if attr.interleave:
                    interleave_attrs.append(attr)
            setattr(entity, "_interleave_", None)
        if interleave_attrs:
            ...
            setattr(entity, "_interleave_", interleave)
        ...
        if len(primary_keys) > 1:
            ...
            pk_attrs = tuple(primary_keys.pop())
        ...
        if len(pk_attrs) == 1:
            pk = pk_attrs[0]
            setattr(entity, "_pk_", pk)
        elif len(pk_attrs) > 1:
            setattr(entity, "_pk_", pk_attrs)
        entity._keys_ = [
        ...
        entity._default_genexpr_ = ast.GeneratorExp(
    ...
    def _check_table_options_(entity) -> None:
        if entity._root_ is not entity:
            if "_table_options_" in entity.__dict__:
                throw(
                    TypeError,
                    "Cannot redefine %s options in %s entity"
                    % (entity._root_.__name__, entity.__name__),
                )
    ...
    def __getitem__(entity, key: _PkValue) -> Entity:
        ...
        if len(key) == len(entity._pk_attrs_):
            ...
            _entity = entity._find_one_(kwargs)
        elif len(key) == len(entity._pk_columns_):
            _entity = entity._get_by_raw_pkval_(key, from_db=False, seed=False)
        else:
            throw(
                TypeError,
    ...
    def select(entity, *args, **kwargs) -> Query:
        if args:
            query = entity._query_from_args_(
                args, kwargs, frame_depth=cut_traceback_depth + 1
            )
        else:
    ...
    def select_random(entity, limit: int) -> list[Entity]:
        ...
        if (
            not issubclass(pk.py_type, int)
            or entity._discriminator_ is not None
            and entity._root_ is not entity
        ):
            return entity.select().random(limit)
        database = entity._database_
        cache = database._get_cache()
    ...
    def _find_one_(
        ...
        if obj is None:
            obj = entity._find_in_db_(avdict, unique, for_update, nowait, skip_locked)
        if obj is None:
    ...
    def _find_in_cache_(
        ...
        if obj is not None:
            ...
            return cast(Entity | None, obj), unique
        return None, unique
            ...
            for i in range(len(col_names)):
                if i not in used_columns:
                    throw(
                        ...
                        % (cursor.description[i][0], entity.__name__),
                    )
        for attr in entity._pk_attrs_:
    ...
    def _construct_select_clause_(
        ...
        for attr in chain(root._attrs_, root._subclass_attrs_):
            if (
                not all_attributes
                and not issubclass(attr.entity, entity)
    ...
    def _construct_batchload_sql_(
        ...
        pc = local.prefetch_context
        attrs_to_prefetch = (
            pc.get_frozen_attrs_to_prefetch(entity) if pc is not None else ()
        )
    ...
    def _construct_sql_(
        ...
        if order_by_pk:
            sql_ast.append(
                ["ORDER_BY"]
                + [["COLUMN", None, column] for column in entity._pk_columns_]
            )
    ...
    def _parse_row_(
        entity, row: DbRow, attr_offsets: AttrOffsets
    ...
    def _select_all(entity) -> Query:
        return Query(
            entity._default_iter_name_, entity._default_genexpr_, {}, {".0": entity}
        )

    def _query_from_args_(
        entity,
        args: tuple[str | Callable | Iterator],
        kwargs: dict[Any, Any],
        frame_depth: int,
    ) -> Query:
        ...
        if type(func) is types.FunctionType:
            ...
            assert isinstance(lambda_ast, ast.Lambda)
        ...
        if len(names) != 1:
            throw(
                ...
                "Got: %d parameters"
                % (entity.__name__, entity.__name__[0].lower(), len(names)),
            )
        name = names[0]
    ...
    def _get_from_identity_map_(
        ...
        if obj is None:
            ...
            object.__setattr__(obj, "__class__", entity)

        if obj is None:
            with cache.flush_disabled():
                obj = obj_to_init
                if obj_to_init is None:
                    obj = cast(Entity, object.__new__(entity))
                assert obj is not None
    ...
    def _get_propagation_mixin_(entity) -> type:
        ...
        for attr in entity._attrs_:
            if not attr.reverse:
                ...
                def fget(wrapper, attr=attr):  # pyright: ignore[reportRedeclaration]
                ...
                def fget(wrapper, attr: Set = attr):
    ...
    def describe(entity) -> str:
        ...
        if entity._pk_is_composite_:
            result.append(
                "PrimaryKey(%s)" % ", ".join(attr.name for attr in entity._pk_attrs_)
            )
        return "\n    ".join(result)
...
class Entity(object, metaclass=EntityMeta):
    ...
    _attrs_with_columns_: Sequence[Attribute]
    _dbvals_: dict[Attribute, OptAttrValue | Entity] | _typeshed.MaybeNone
    _newid_: int | None
    _pkval_: _PkValue | _typeshed.MaybeNone
    _rbits_: int | _typeshed.MaybeNone
    _save_pos_: int | None
    _session_cache_: SessionCache | _typeshed.MaybeNone
    _status_: str | None
    _vals_: dict[Attribute, Any] | _typeshed.MaybeNone
    _wbits_: int | _typeshed.MaybeNone

    def __reduce__(obj):
        if obj._status_ in del_statuses:
            throw(
                OperationWithDeletedObjectError,
                "Deleted object %s cannot be pickled" % safe_repr(obj),
            )
    ...
    def _get_raw_pkval_(obj) -> tuple[_PkValue, ...]:
        pkval = obj._pkval_
        if not obj._pk_is_composite_:
            if not obj._pk_attrs_[0].reverse:
                return (pkval,)
            else:
                assert isinstance(pkval, Entity)
    ...
    def _load_(obj) -> None:
        cache = obj._session_cache_
        if cache is None or not cache.is_alive:
            throw_db_session_is_over("load object", obj)
        entity = obj.__class__
        database = entity._database_
        if cache is not database._get_cache():
            throw(
        ...
        if obj not in objects:
            throw(
                UnrepeatableReadError, "Phantom object %s disappeared" % safe_repr(obj)
            )
    ...
    def load(obj, *attrs) -> None:
            ...
            for arg in args:
                if isinstance(arg, str):
                    ...
                    if attr is None:
                        ...
                        throw(
                            AttributeError,
                            "Object %s does not have attribute %r" % (obj, arg),
                        )
                elif isinstance(arg, Attribute):
                    attr = arg
        ...
        if cached_sql is None:
            ...
            criteria_list = [
                ...
                for i, (column, converter) in enumerate(
                    zip(obj._pk_columns_, obj._pk_converters_)
                )
            ]
    ...
    def _db_set_(obj, avdict: _AvDict, unpickling: bool = False) -> None:
        ...
        if unpickling:
            ...
            new_dbvals = {
                attr: (
                    attr.converters[0].val2dbval(val, obj) if not attr.reverse else val
                )
                for attr, val in avdict.items()
            }
        else:
            ...
            new_vals = {
                attr: (
                    attr.converters[0].dbval2val(dbval, obj)
                    if not attr.reverse
                    else dbval
                )
            ...
            if any(attr in new_vals for attr in attrs):
                ...
                if prev_key_vals != new_key_vals:
                    cache.db_update_composite_index(
                        obj, attrs, prev_key_vals, new_key_vals
                    )
            ...
            try:
                ...
                for attr in obj._attrs_:
                    if not attr.is_collection:
                        ...
                        if not reverse.is_collection:
                            val = (
                                get_val(attr) if attr in obj._vals_ else attr.load(obj)
                            )
                            if val is None:
                                continue
                            if attr.cascade_delete:
    ...
    def delete(obj) -> None:
        cache = obj._session_cache_
        if cache is None or not cache.is_alive:
            throw_db_session_is_over("delete object", obj)
        obj._delete_()
    ...
    def set(obj, **kwargs) -> None:
        cache = obj._session_cache_
        if cache is None or not cache.is_alive:
            throw_db_session_is_over("change object", obj)
        if obj._status_ in del_statuses:
            throw_object_was_deleted(obj)
        with cache.flush_disabled():
            ...
            if avdict:
                if any(
                    attr not in obj._vals_ and attr.reverse and obj._bits_[attr]
                    for attr in avdict
                ):
                    obj._load_()
    ...
    def _update_dbvals_(
        obj, after_create: bool, new_dbvals: dict[Attribute, AttrValue | Self]
    ) -> None:
        bits = obj._bits_
        vals = obj._vals_
        dbvals = obj._dbvals_
    ...
    def _save_updated_(obj) -> None:
        ...
        if update_columns:
            ...
            if (
                cursor.rowcount == 0
                and cache.db_session
                and cache.db_session.optimistic
            ):
    ...
    def flush(obj) -> None:
        ...
        assert obj._save_pos_ is not None, (
            "save_pos is None for %s object" % obj._status_
        )
        cache = obj._session_cache_
    ...
    def to_json(
        ...
        return obj._database_.to_json(
            obj, include, exclude, converter, with_schema, schema_hash
        )
...
def get_globals_and_locals(
    args: (
        tuple[str | types.FunctionType]  # noqa: Y090
        | tuple[str | types.FunctionType, dict[str, object]]
        | tuple[str | types.FunctionType, dict[str, object], dict[str, object]]
    ),
    /,
...
) -> tuple[str | types.GeneratorType, dict[str, object], dict[str, object]]: ...
...
count: Callable[..., int] = make_aggrfunc(utils.count)
sum: Callable[..., Numeric] = make_aggrfunc(builtins.sum)
min: Callable = make_aggrfunc(builtins.min)
max: Callable = make_aggrfunc(builtins.max)
avg: Callable[..., float] = make_aggrfunc(utils.avg)
group_concat: Callable[..., str | None] = make_aggrfunc(utils.group_concat)

distinct: Callable = make_aggrfunc(utils.distinct)
...
def extract_vars(
    ...
    cells: dict[str, types.CellType] | None = None,
    ...
    vars = {}
    vartypes: VarTypes = HashableDict()
    for src, extractor in extractors.items():
        ...
        if src == "False" and value is not False:
            throw(TranslationError)
        try:
            vartypes[varkey], value = normalize(value)  # type: ignore
        except TypeError:
...
class Query(object):
    ...
    def __init__(
        ...
        varkey = filter_num, node.src, code_key  # type: ignore[attr-defined]
        origin = vars[varkey]
        if isinstance(origin, Query):
            ...
            if not isinstance(origin, EntityMeta):
                if node.src == ".0":  # type: ignore[attr-defined]
                    throw(
                        TypeError,
                        "Query can only iterate over entity or another query (not a list of objects)",
                    )
                throw(TypeError, "Cannot iterate over non-entity object %s" % node.src)  # type: ignore[attr-defined]
    ...
    def __reduce__(query) -> tuple[Callable, tuple[QueryResult]]:
    ...
    def _construct_sql_and_arguments(
        ...
        if cache_entry is None:
            ...
            sql, adapter, attr_offsets = cast(_CachedSql, cache_entry)
        ...
        if query._translator.query_result_is_cacheable:
            ...
            query_key = None
        return sql, arguments, cast(AttrOffsets | None, attr_offsets), query_key
    ...
    def _actual_fetch(
        ...
        with query._prefetch_context:
            ...
            if items is None:
                cursor = database._exec_sql(sql, arguments)
                assert translator.row_layout is not None
    ...
    @cut_traceback
    def show(query, width: int | None = None, stream: StringIO | None = None) -> None:
    ...
    def first(query) -> Any:
        ...
        if translator.order:
            ...
            query = query.order_by(
                *[i + 1 for i in range(len(query._translator.expr_type))]
            )
        else:
    ...
    def delete(query, bulk: bool | None = None) -> int:
        ...
        if cache_entry is None:
            ...
            database._constructed_sql_cache[sql_key] = cache_entry
        sql, adapter = cast(CachedSqlSimple, cache_entry)
        arguments = adapter(query._vars)
        cache.immediate = True
        cache.prepare_connection_for_query_execution()  # may clear cache.query_results
        cursor = database._exec_sql(sql, arguments)
    ...
    @cut_traceback
    def sort_by(query, *args) -> Query:
        return query._order_by("sort_by", *args)

    def _order_by(query, method_name: str, *args) -> Query:
        if not args:
    ...
    def _process_lambda(
        ...
        if extractors:
            ...
            new_vars, vartypes = query._vars, HashableDict()
        tup = (
            (
    ...
    def where(query, *args, **kwargs):
        if args:
            ...
            return query._process_lambda(
                func, globals, locals, order_by=False, original_names=True
            )
        if not kwargs:
            return query

        if len(query._translator.tree.generators) > 1:
            throw(
                TypeError,
    ...
    def _aggregate(
        query, aggr_func_name: str, distinct: bool | None = None, sep: str | None = None
    ) -> Any:
        translator = query._translator
        sql, arguments, attr_offsets, query_key = query._construct_sql_and_arguments(
    ...
    def to_json(
        ...
        return query._database.to_json(
            query[:], include, exclude, converter, with_schema, schema_hash
        )
...
class QueryResult(object):
    ...
    def __init__(
        ...
        self._expr_type = cast(ExprType, translator.expr_type)
    ...
    def _get_type_(self) -> QueryType | tuple[ExprType, ...]:
        ...
        item_type = cast(ExprType, self._query._translator.expr_type)
        return tuple(item_type for _ in self._items)
    ...
    def _normalize_var(
        ...
        item_type = cast(ExprType, self._query._translator.expr_type)
        return tuple(item_type for _ in items), items
