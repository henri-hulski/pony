...
from pony.orm.core import Database, Entity, TransactionError
...
class Bag(object):
    ...
    @cut_traceback
    def to_dict(bag) -> defaultdict[str, dict[int, dict[str, int | str | list | None]]]:
        bag.dicts.clear()
        ...
        for attr in attrs:
            ...
            if attr.is_collection:
                ...
                if attr.reverse.entity._pk_is_composite_:
                    value = sorted(
                        bag._reduce_composite_pk(item._get_raw_pkval_())
                        for item in value
                    )
                else:
    ...
    def to_json(bag):
        return json.dumps(
            bag.to_dict(), default=json_converter, indent=2, sort_keys=True
        )
...
def to_dict(
    ...
    if isinstance(objects, Entity):
        objects = [objects]
    objects = iter(objects)  # type: ignore
    try:
