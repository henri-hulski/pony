...
def isbn10_checksum(digits):
    if len(digits) != 9:
        raise ValueError()
    reminder = (
        sum(digit * coef for digit, coef in zip(map(int, digits), range(10, 1, -1)))
        % 11
    )
    if reminder == 1:
        return "X"
...
def isbn13_checksum(digits):
    if len(digits) != 12:
        raise ValueError()
    reminder = (
        sum(digit * coef for digit, coef in zip(map(int, digits), (1, 3) * 6)) % 10
    )
...
email_re = re.compile(
    r"^[a-z0-9._%+-]+@[a-z0-9][a-z0-9-]*(?:\.[a-z0-9][a-z0-9-]*)+$", re.IGNORECASE
)

rfc2822_email_re = re.compile(
    r"""
    ^(?: [a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*
...
datetime_re_list = [
    re.compile(r"^%s(?:[t ]%s)?$" % (date_str, time_str), re.UNICODE | re.VERBOSE)
...
def str2py(value, type):
    if type is None or not isinstance(value, str):
        return value
    if isinstance(type, tuple):
        str2py, py2str, err_msg = type
    else:
        str2py, py2str, err_msg = converters.get(type, (type, str, None))
    try:
        return str2py(value)
